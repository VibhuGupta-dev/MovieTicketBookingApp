...
export function Profile() {
  ...
  const [ticket, setTicket]       = useState(null)   // fetched ticket data
  ...
  const openTicket = async (ticketId) => {
    ...
    try {
      const res = await axios.get(`http://localhost:3000/ticket/api/${ticketId}`, { withCredentials: true })
      console.log(res)
      const data = res.data.yticket ?? res.data.ticket ?? res.data
      setTicket(data)
    } catch (err) {
      setTicketError("Ticket load nahi hua." , err)
    } finally {
            ...
            <div className="px-6 pb-8 pt-4">
              ...
              {loadingTicket && (
                <div className="animate-pulse space-y-4">
                  <div className="mx-auto w-48 h-48 rounded-2xl bg-white/8" />
              ...
              {!loadingTicket && ticket && (
                <>
                  ...
                  {/* Perforation */}
                  <div className="flex items-center gap-0 my-5 -mx-6">
                  ...
                  {/* Details — exact fields from DB */}
                  <div className="space-y-0 divide-y divide-white/[0.05]">
                    <div className="flex justify-between py-3">
                      <span className="text-[11px] uppercase tracking-wider text-white/25">Ticket ID</span>
                      <span className="font-mono text-[11px] text-white/50">#{String(ticket._id).slice(-8).toUpperCase()}</span>
                    </div>
                    <div className="flex justify-between py-3">
                      <span className="text-[11px] uppercase tracking-wider text-white/25">Seats</span>
                      <span className="text-sm font-semibold text-white/80">
                        {ticket.SeatIds?.length ?? 0}
                        <span className="font-normal text-white/35 ml-1">seat{ticket.SeatIds?.length !== 1 ? "s" : ""}</span>
                      </span>
                    </div>
                    <div className="flex justify-between py-3">
                    ...
                    <div className="flex justify-between py-3">
                      ...
                      <span className="text-sm text-white/60">{ticket.bookedAt ? `${fmt.date(ticket.bookedAt)}, ${fmt.time(ticket.bookedAt)}` : "—"}</span>
                    </div>
                    <div className="flex justify-between py-3">
                      <span className="text-[11px] uppercase tracking-wider text-white/25">QR Status</span>
                      {ticket.isQRgenerated
                        ? <span className="flex items-center gap-1.5 text-[11px] font-semibold text-emerald-400"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />Generated</span>
                        : <span className="flex items-center gap-1.5 text-[11px] font-semibold text-amber-400"><span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />Pending</span>
                      }
                    </div>
                  </div>
                </>
            ...
            {history.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  className={`
                    group flex items-center gap-3.5 rounded-2xl px-5 py-3.5 border transition-all duration-200
                ...
                >
                  ...
                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    <span className="text-[15px] font-bold text-white/90 tabular-nums">{fmt.amount(item.totalAmount)}</span>
                    <div className="flex items-center gap-1.5">
                      <span className={`inline-flex items-center gap-1 px-2 py-[3px] rounded-full text-[10.5px] font-semibold leading-none ${
                        isPaid ? "bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/20" : "bg-amber-500/10 text-amber-400 ring-1 ring-amber-500/20"
                      }`}>

...
function EditModal({ cinema, onClose, onSaved }) {
  ...
  const inputCls =
    "w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all";
  const labelCls =
    "block text-xs font-semibold uppercase tracking-widest text-gray-500 mb-1.5";
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm font-semibold rounded-xl transition-all"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-semibold rounded-xl transition-all shadow-lg shadow-indigo-900/30"
            >
...
function DeleteModal({ cinema, onClose, onDeleted }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `${API}/cinemahall/api/deletecinemahall/${cinema._id}`,
        authHeaders(),
      );
      onDeleted();
    } catch (err) {
    ...
    >
      <div className="w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-8 relative">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          <div className="h-16 w-16 bg-red-500/15 rounded-2xl flex items-center justify-center text-red-400 mb-4">
            {Icons.Trash("h-8 w-8")}
          </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 py-3 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white text-sm font-semibold rounded-xl transition-all shadow-lg shadow-red-900/30"
          >
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden hover:border-gray-700 transition-all duration-200 group">
      <div className="relative h-40 bg-gray-800 overflow-hidden">
        {cinema.logo ? (
          <img
            src={cinema.logo}
            alt={cinema.cinemaHallName}
            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-gray-700">
            {Icons.Theater("h-16 w-16 opacity-40")}
          </div>
        )}
      ...
      <div className="p-5">
        ...
        <p className="text-gray-500 text-xs truncate mb-3">
          {cinema.address || "No address set"}
        </p>
        <p className="text-gray-400 text-xs line-clamp-2 mb-4 leading-relaxed">
          {cinema.description || "No description"}
        </p>
        <div className="flex items-center gap-3 mb-4">
          {[
            ["Rows", cinema.row || 0],
            ["Per Row", cinema.seatsPerRow || 0],
            ["Seats", totalSeats],
          ].map(([lbl, val]) => (
            <div
              key={lbl}
              className="flex-1 bg-gray-800 rounded-xl px-3 py-2 text-center"
            >
...
function AddCinemaModal({ onClose, onAdded }) {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/countries/IN/states/${selectedState.iso2}/cities`, {
      headers: { "X-CSCAPI-KEY": API_KEY },
    })
      .then((r) => r.json())
  ...
  const handleStateSelect = (s) => {
    setSelectedState(s);
    setForm((f) => ({ ...f, StateId: String(s.id), CityId: "" }));
    setStateOpen(false);
    setStateSearch("");
    setCitySearch("");
  };
  const handleCitySelect = (c) => {
    setSelectedCity(c);
    setForm((f) => ({ ...f, CityId: String(c.id) }));
    setCityOpen(false);
    setCitySearch("");
  };
  const handleChange = (e) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post(
        `${API}/cinemahall/api/addchinemahall`,
        form,
        authHeaders(),
      );
      onAdded();
    } catch (err) {
      setError(err?.response?.data?.message || "Failed to add cinema");
    } finally {
      setLoading(false);
    }
  };
  ...
  const labelCls =
    "block text-xs font-semibold uppercase tracking-widest text-gray-500 mb-1.5";
  const selectBtnCls =
    "w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-sm outline-none transition-all flex items-center justify-between cursor-pointer hover:border-gray-600";
            ...
            items.map((item) => (
              ...
              >
                ...
                {selected?.[keyField] === item[keyField] &&
                  Icons.Check("h-3.5 w-3.5 text-indigo-400")}
              </button>
            ))
          )}
        </div>
    ...
    >
      <div className="w-full max-w-lg bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl relative max-h-[92vh] overflow-y-auto">
        <div className="sticky top-0 bg-gray-900 border-b border-gray-800 px-8 pt-7 pb-5 z-10">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="h-10 w-10 bg-green-600/20 rounded-xl flex items-center justify-center text-green-400">
              {Icons.Plus("h-5 w-5")}
            </div>
            <div>
              ...
              <p className="text-gray-500 text-xs">
                Step {step} of 2 —{" "}
                {step === 1 ? "Basic Info" : "Layout & Pricing"}
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows={3}
                  placeholder="Describe your cinema hall…"
                  className={`${inputCls} resize-none`}
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    <span
                      className={
                        selectedState ? "text-gray-200" : "text-gray-600"
                      }
                    >
                      {selectedState
                        ? selectedState.name
                    ...
                    {Icons.ChevronDown(
                      `h-4 w-4 text-gray-500 transition-transform ${stateOpen ? "rotate-180" : ""}`,
                    )}
                  </button>
                  {stateOpen && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl z-20 overflow-hidden">
                      ...
                      {/* ✅ Render list directly, NOT via SearchDropdown */}
                      <div className="max-h-48 overflow-y-auto">
                        {filteredStates.length === 0 ? (
                          ...
                          filteredStates.map((item) => (
                            <button
                              ...
                              className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between ${selectedState?.iso2 === item.iso2 ? "bg-indigo-600/30 text-indigo-300" : "text-gray-300 hover:bg-gray-700"}`}
                            >
                              {item.name}
                              {selectedState?.iso2 === item.iso2 &&
                                Icons.Check("h-3.5 w-3.5 text-indigo-400")}
                            </button>
                          ))
                        )}
              ...
              <div>
                ...
                <div className="relative">
                  <button
                    ...
                    className={`${selectBtnCls} ${!selectedState ? "opacity-50 cursor-not-allowed" : ""}`}
                  >
                    <span
                      className={
                        selectedCity ? "text-gray-200" : "text-gray-600"
                      }
                    >
                      {!selectedState
                    ...
                    {Icons.ChevronDown(
                      `h-4 w-4 text-gray-500 transition-transform ${cityOpen ? "rotate-180" : ""}`,
                    )}
                  </button>
                  {cityOpen && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl z-20 overflow-hidden">
                        ...
                        {filteredCities.length === 0 ? (
                          ...
                          filteredCities.map((item) => (
                            <button
                              ...
                              className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between ${selectedCity?.id === item.id ? "bg-indigo-600/30 text-indigo-300" : "text-gray-300 hover:bg-gray-700"}`}
                            >
                              {item.name}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                {[
                  ["row", "Rows *", "10", "1", "50"],
                  ["seatsPerRow", "Seats/Row *", "15", "1", ""],
                  ["RatePerSeat", "Rate (₹) *", "200", "1", ""],
                ].map(([name, lbl, ph, min, max]) => (
                  <div key={name}>
                  ...
                  <div className="flex flex-col items-center gap-1 max-h-36 overflow-hidden">
                    {Array.from({ length: Math.min(rows, 8) }).map((_, r) => (
                      <div key={r} className="flex gap-1">
                        {Array.from({ length: Math.min(cols, 20) }).map(
                          (_, c) => (
                            <div
                              key={c}
                              className="h-2.5 w-2.5 rounded-sm bg-indigo-500/60"
                            />
                          ),
                        )}
                        {cols > 20 && (
                  ...
                  {form.RatePerSeat && (
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <span className="text-green-400 font-bold text-sm">
                        ₹
                        {(
                          totalPreview * parseInt(form.RatePerSeat)
                        ).toLocaleString()}
                      </span>
                    </div>
                  )}
              ...
              <div className="flex gap-3 pt-1">
                ...
                >
                  {loading ? (
                    ...
                    <>{Icons.Check("h-4 w-4")} Create Cinema</>
                  )}
                </button>
              </div>
            </div>
...
function AddShowModal({ cinema, onClose, onAdded }) {
  ...
  const handleSubmit = async () => {
    if (!selectedMovie || !form.showDate || timeSlots.length === 0) {
      setError("Please fill all fields and add at least one time slot.");
      return;
    }
    setLoading(true);
    setError("");
    try {
      await axios.post(
        `${API}/show/addshow/${cinema._id}/${selectedMovie._id}`,
        {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-8 relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="h-10 w-10 bg-purple-600/20 rounded-xl flex items-center justify-center text-purple-400">
            {Icons.Film("h-5 w-5")}
          </div>
          <div>
            <h2 className="text-white font-bold text-lg">Add Show</h2>
            <p className="text-gray-500 text-xs">{cinema.cinemaHallName}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                <span
                  className={selectedMovie ? "text-gray-200" : "text-gray-600"}
                >
                  {moviesLoading
                    ? "Loading movies…"
                    : selectedMovie
                ...
                {Icons.ChevronDown(
                  `h-4 w-4 text-gray-500 transition-transform ${movieOpen ? "rotate-180" : ""}`,
                )}
              </button>
              {movieOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl z-20 overflow-hidden">
                      ...
                      filteredMovies.map((m) => (
                        ...
                        >
                          {m.MoviePhoto ? (
                            <img
                              src={m.MoviePhoto}
                              className="h-8 w-6 object-cover rounded flex-shrink-0"
                            />
                          ) : (
                            <div className="h-8 w-6 bg-gray-700 rounded flex-shrink-0 flex items-center justify-center text-gray-500">
                              {Icons.Film("h-3 w-3")}
                            </div>
                          )}
                          <span>{m.MovieName}</span>
                          {selectedMovie?._id === m._id &&
                            Icons.Check("h-3.5 w-3.5 text-purple-400 ml-auto")}
                        </button>
                      ))
                    )}
                  </div>
            ...
            {timeSlots.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {timeSlots.map((t) => (
                  <span
                    key={t}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600/20 border border-purple-600/30 text-purple-300 text-xs rounded-lg"
                  >
...
function EditShowModal({ show, onClose, onSaved }) {
  ...
  const addTimeSlot = () => {
    const t = timeInput.trim();
    if (t && !form.timeSlots.includes(t)) {
      setForm((f) => ({ ...f, timeSlots: [...f.timeSlots, t] }));
      setTimeInput("");
    }
  };
  const removeTimeSlot = (t) =>
    setForm((f) => ({ ...f, timeSlots: f.timeSlots.filter((x) => x !== t) }));
    ...
    >
      <div className="w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-8 relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-white font-bold text-lg">Edit Show</h2>
            <p className="text-gray-500 text-xs">
            ...
            {form.timeSlots.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {form.timeSlots.map((t) => (
                  <span
                    key={t}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600/20 border border-indigo-600/30 text-indigo-300 text-xs rounded-lg"
                  >
                    {Icons.Clock("h-3 w-3")} {t}
                    <button
                      onClick={() => removeTimeSlot(t)}
...
function ShowsPanel({ cinema, onClose }) {
  ...
  const fetchShows = async () => {
    ...
    try {
      const res = await axios.get(
        `${API}/show/getshowbycinema/${cinema._id}`,
        authHeaders(),
      );
      setShows(res.data || []);
    } catch {
  ...
  const handleDelete = async (showId) => {
    ...
    try {
      await axios.delete(`${API}/show/deleteshow/${showId}`, authHeaders());
  ...
  const grouped = shows.reduce((acc, show) => {
    const date = new Date(show.showDate).toLocaleDateString("en-IN", {
      weekday: "short",
      day: "numeric",
      month: "short",
      year: "numeric",
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-50 flex items-end sm:items-center justify-center backdrop-blur-md bg-black/70 px-0 sm:px-4"
        onClick={(e) => e.target === e.currentTarget && onClose()}
      >
        <div className="w-full sm:max-w-2xl bg-gray-900 border border-gray-800 rounded-t-2xl sm:rounded-2xl shadow-2xl relative max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-800 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-bold text-base">Shows</h2>
                <p className="text-gray-500 text-xs">{cinema.cinemaHallName}</p>
              </div>
            </div>
                  ...
                  <div key={date}>
                    ...
                    <div className="space-y-3">
                      {dateShows.map((show) => (
                        <div
                          key={show._id}
                          className="bg-gray-800/60 border border-gray-700 rounded-xl p-4 flex items-start justify-between gap-3"
                        >
                          <div className="flex-1 min-w-0">
                            <p className="text-white text-sm font-semibold truncate mb-2">
                              {show.movieId?.MovieName || "Unknown Movie"}
                            </p>
                            <div className="flex flex-wrap gap-1.5">
                              {(show.timeSlots || []).map((slot, i) => (
                                <span
                                  key={i}
                                  className="flex items-center gap-1 px-2.5 py-1 bg-purple-600/20 border border-purple-600/30 text-purple-300 text-xs rounded-lg"
                                >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center backdrop-blur-sm bg-black/50 px-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex flex-col items-center text-center mb-5">
              <div className="h-14 w-14 bg-red-500/15 rounded-2xl flex items-center justify-center text-red-400 mb-3">
                {Icons.Trash("h-7 w-7")}
              </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm font-semibold rounded-xl transition-all"
              >
                Cancel
              </button>
              <button
...
export function OwnerPage() {
  ...
  const fetchCinemas = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API}/cinemahall/api/mycinemas`,
        authHeaders(),
      );
      setCinemas(res.data?.cinemas || []);
    } catch {
      setError("Failed to load your cinemas.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleted = () => {
    setDeleteTarget(null);
    showToast("Cinema deleted.");
    fetchCinemas();
  };
  const handleAdded = () => {
    setShowAdd(false);
    showToast("Cinema added successfully!");
    fetchCinemas();
  };

  const totalSeats = cinemas.reduce(
    (acc, c) => acc + (c.seats?.length || 0),
    0,
  );
  const totalMovies = cinemas.reduce(
    (acc, c) => acc + (c.MovieId?.length || 0),
    0,
  );
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <StatCard
            ...
            icon={Icons.Theater("h-6 w-6")}
            color="bg-indigo-500/15 text-indigo-400"
          />
          <StatCard
            label="Total Seats"
            value={totalSeats}
            icon={Icons.Seat("h-6 w-6")}
            color="bg-purple-500/15 text-purple-400"
          />
          <StatCard
            label="Movies"
            value={totalMovies}
            icon={Icons.Movie("h-6 w-6")}
            color="bg-pink-500/15 text-pink-400"
          />
          <StatCard
            label="Status"
            value="Active"
            icon={Icons.Check("h-6 w-6")}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm rounded-xl transition-all"
            >
              Try Again
            </button>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-gray-600 text-sm mb-2">
              Get started by adding your first cinema hall.
            </p>
            <button
              onClick={() => setShowAdd(true)}
      ...
      {showsTarget && (
        <ShowsPanel cinema={showsTarget} onClose={() => setShowsTarget(null)} />
      )}
      {showAdd && (
        <AddCinemaModal

...
export function Allshows() {
  ...
  // Each movie card calls this with its own movieId and movieName
  const handleMovieClick = (movieId, movieName) => {
    navigate(`/${movieId}/cinemas/${today}`, {
      state: { movieId, movieName }
    });
  }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {movies.map(movie => (
                <MovieCard
                  key={movie._id}
                  movie={movie}
                  onBook={() => handleMovieClick(movie._id, movie.MovieName)}
                />
              ))}
            </div>
          )}
...
function MovieCard({ movie, onBook }) {
  ...
  return (
    <div
      onClick={onBook}
      className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden hover:-translate-y-1 hover:border-purple-700 hover:shadow-lg hover:shadow-purple-900/30 transition-all duration-200 cursor-pointer group"
    >
      {/* Poster */}
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-gradient-to-r from-purple-700 to-purple-500 hover:opacity-85 text-white text-xs font-semibold rounded-lg transition-opacity"
          >
            Book Now
          </button>
        </div>
